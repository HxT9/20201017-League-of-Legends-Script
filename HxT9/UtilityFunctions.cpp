...
namespace UtilityFunctions{
	...
	void Init()
	{
		...
		if (!ChatOpenAddress) {
			IMGUIManager::Print("Can't find isChatOpen offset");
		}
	}

	void drawMyHero() {
		if (myHero.PCObject) {
			D3DDrawer::DrawCircumference(myHero.Pos, myHero.BoundingRadius, 15, 0xff00ff00, 2);
		}
	}

	float CalcEffectiveDamage(float damage, float armor) { //Anche per la magicRes
		return damage * (100 / (100 + armor));
	}

	bool IsValidTarget(EntityBase* target) {
		return target->Visible && target->Targetable && GameFunctions::IsAlive(target->PCObject);
	}
	...
	void drawEntities() {
		...
		for (int i = 0; i < EntitiesContainer::HeroesIndex.size(); i++) {
			...
			if (IsValidTarget(temp)) {
				if (temp->Team != myHero.Team) {
					...
					D3DDrawer::DrawCircumference(temp->Pos, r, 30, 0x7fff0000, 2);

					if (temp->Pos.distTo(myHero.Pos) > 800 && temp->Pos.distTo(myHero.Pos) < 2500)
						D3DDrawer::DrawLine(temp->Pos, myHero.Pos, 0xffff0000, 2);

					if (temp->Index == myHero.SelectedTargetIndex) {
						D3DDrawer::DrawCircumference(temp->Pos, 75, 20, 0xffffff00, 4);
					}
				}
			}
		}
		...
		for (int i = 0; i < EntitiesContainer::TurretsIndex.size(); i++) {
			...
			if (temp->Team != myHero.Team && temp->Visible && temp->Health > 1 && temp->Pos.isDrawable()) {
				D3DDrawer::DrawCircumference(temp->Pos, 850, 30, 0xffff0000, 2);
			}
		}

		D3DDrawer::DrawCircumference(myHero.Pos, myHero.AttackRange + myHero.BoundingRadius, 50, 0x7f00ff00, 2);
	}
	...
	void drawLastHittableMinions() {
		...
		for (int i = 0; i < EntitiesContainer::MinionsIndex.size(); i++) {
			...
			if (effDamage > temp->Health) {
				D3DDrawer::DrawCircumference(temp->Pos, 50, 10, 0x7f00ff00, 2);
			}
			else if (effDamage > temp->Health - dmgIncoming) {
				D3DDrawer::DrawCircumference(temp->Pos, 50, 10, 0x7fffff00, 2);
			}
			else {
				D3DDrawer::DrawCircumference(temp->Pos, 50, 10, 0x7fff0000, 2);
			}
		}
	}
	...
	void drawDebug() {
		...
		for (int i = 0; i < EntitiesContainer::MaxIndex; i++) {
			...
			if (temp->PCObject) {
				...
				D3DDrawer::DrawTextSmall(pos, UtilityFunctions::Stringf("%s", str.c_str()).c_str(), 0xffffffff);
			}
		}

		Path p = myHero.PCObject->GetPath();
		for (int i = 0; i < p.nPathPoints; i++) {
			D3DDrawer::DrawCircumference(p.pathPoints[i], 30, 10, 0xffffff00, 2);

			if (i > 0)
				D3DDrawer::DrawLine(p.pathPoints[i], p.pathPoints[i - 1], 0xffffffff, 2);
		}

		for (int i = 0; i < EntitiesContainer::HeroesIndex.size(); i++) {
			D3DDrawer::DrawCircumference(GetPredictedPos(EntitiesContainer::Entities[EntitiesContainer::HeroesIndex[i]], 0.5, 145), 30, 10, 0xffffff00, 2);
		}
	}

	bool IsCollision(Vector3 start, Vector3 end, float SpellWidth, Vector3 center, float radius)
	{
	...
	Vector3 GetPredictedPos(EntityBase* hero, float seconds, float width, SkillShotType spellType) {
		int firstPoint = 0;
		...
		if (myHero.ObjectName == "Xerath") {
			...
			for (int i = 0; i < EntitiesContainer::HeroesIndex.size(); i++) {
				...
				if (IsValidTarget(temp) && temp->IsEnemyTo(&myHero) && temp->Health < CalcEffectiveDamage(dmg, temp->MagicResist)) {
					GameFunctions::WorldToScreen(&temp->Pos, &screenPos);
					D3DDrawer::DrawTextSmall(screenPos, "!!! KILLABLE WITH ULT !!!", 0xffffff00);
				}
			}
		}
			...
			for (int i = 0; i < EntitiesContainer::HeroesIndex.size(); i++) {
				...
				if (dmg > 0) {
					if (IsValidTarget(temp) && temp->IsEnemyTo(&myHero) && temp->Health > 0) {
						D3DDrawer::DrawTextSmall(temp->Pos.ToScreen(), Stringf("%d%%", (int)(dmg * 100 / temp->Health)).c_str(), 0xffffffff);
					}
				}
			}
		}
	...
	void drawSpellCD() {
		...
		for (int i = 0; i < EntitiesContainer::HeroesIndex.size(); i++) {
			temp = EntitiesContainer::Entities[EntitiesContainer::HeroesIndex[i]];
			if (temp->Index != myHero.Index && temp->Visible && GameFunctions::IsAlive(temp->PCObject)) {
				GameFunctions::WorldToScreen(&temp->Pos, &screenPos);
				screenPos.y -= 100;
				...
				screenPos.x += 25;
				D3DDrawer::DrawTextMedium(screenPos, text.c_str(), !temp->SpellQCD ? 0xff00ff00 : 0xffff0000);

				text = Stringf("[%d]", (int)ceilf(temp->SpellWCD));
				screenPos.x += 35;
				D3DDrawer::DrawTextMedium(screenPos, text.c_str(), !temp->SpellWCD ? 0xff00ff00 : 0xffff0000);

				text = Stringf("[%d]", (int)ceilf(temp->SpellECD));
				screenPos.x += 35;
				D3DDrawer::DrawTextMedium(screenPos, text.c_str(), !temp->SpellECD ? 0xff00ff00 : 0xffff0000);

				text = Stringf("[%d]", (int)ceilf(temp->SpellRCD));
				screenPos.x += 35;
				D3DDrawer::DrawTextMedium(screenPos, text.c_str(), !temp->SpellRCD ? 0xff00ff00 : 0xffff0000);

				text = Stringf("[%d]", (int)ceilf(temp->SpellDCD));
				screenPos.x += 35;
				D3DDrawer::DrawTextMedium(screenPos, text.c_str(), !temp->SpellDCD ? 0xff00ffff : 0xffffff00);

				text = Stringf("[%d]", (int)ceilf(temp->SpellFCD));
				screenPos.x += 35;
				D3DDrawer::DrawTextMedium(screenPos, text.c_str(), !temp->SpellFCD ? 0xff00ffff : 0xffffff00);
			}
		}
	}
	...
	void drawRectangle(Vector3 vStart, Vector3 vEnd, float radius) {
		D3DDrawer::DrawRectangle(vStart, vEnd, radius, 0xffffffff, 2);
	}

	void drawCircular(Vector3 vCenter, float radius) {
		D3DDrawer::DrawCircumference(vCenter, radius, 15, 0xffffffff, 2);
	}

	void drawConic(Vector3 vCenter, Vector3 vEnd, int angle) {
		D3DDrawer::DrawConic(vCenter, vEnd, angle, 15, 0xffffffff, 2);
	}
